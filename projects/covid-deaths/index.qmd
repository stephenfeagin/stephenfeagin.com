...
I made a [Tableau story](https://public.tableau.com/shared/M247NGXYW?:display_count=n&:origin=viz_share_link) from this exploratory data analysis. Check it out!
...
lm(deaths_per_thousand ~ I(elderly_pop * 100), data = joined_deaths_and_elderly) |> 
  stargazer(type = "html")
```
A fairly strong positive association: for an increase in elderly population of one percentage point, we can expect an additional 0.075 COVID deaths per thousand residents, with p < 0.01.
...
To get the data into Tableau, I write out a few CSV files:

```{r}
#| exec: false

write_csv(deaths_long, file = "covid19_deaths_long.csv")
write_csv(income, file = "cleaned_income.csv")

health_covariates <- elderly_pop |> 
  inner_join(physicians) |> 
  inner_join(smokers)
write_csv(health_covariates, file = "health_covariates.csv")
```
